...
* A [customer](https://docs.y.uno/reference/create-customer) created in Yuno and a [checkout session](https://docs.y.uno/reference/create-checkout-session): reference each API when setting up your backend.
* Public API key (obtain from the [Yuno Dashboard](https://dashboard.y.uno/) → **Developers** > **Credentials**)

## Include the library in your project

<Note>
  The web SDK supports Subresource Integrity (SRI), see the [common reference](https://docs.y.uno/docs/web-sdk-common-reference#subresource-integrity-sri) for details.
</Note>
...
| `yunoPaymentResult(status)`       | Callback: payment finished. Receives [payment status](https://docs.y.uno/reference/payment) (e.g. `SUCCEEDED`, `DECLINED`, `PENDING`). |
...
<Frame>![Seamless (payment Web) screenshot](/images/reference/payment-flows-web/image1.png)</Frame>
...
Create a `checkout_session` using the [Create checkout session](https://docs.y.uno/reference/create-checkout-session) endpoint. If your workflow requires sending the `additional_data` object, include it in the checkout session.

* [Create a customer](https://docs.y.uno/reference/create-customer) or retrieve an existing customer ID
...
See the [Payment type](https://docs.y.uno/reference/payment-type-list) page to view the complete list of payment method types you can use when mounting the SDK.
...
For more information about enrolling payment methods, see [Enroll Payment Methods](https://docs.y.uno/docs/enroll-payment-methods).
...
* Create a [One-Time Use Token](https://docs.y.uno/docs/tokens) using credit card information, then create a payment.
* Create a [One-Time Use Token](https://docs.y.uno/docs/tokens) using a `vaulted_token` from a previously enrolled credit card, then create a payment.
