...
* A [customer](https://docs.y.uno/reference/create-customer) created in Yuno, a [customer session](https://docs.y.uno/reference/create-customer-session), and an [enrollment payment method object](https://docs.y.uno/reference/enroll-payment-method-checkout): reference each API when setting up your backend.
...
Create a [customer session](https://docs.y.uno/reference/create-customer-session) and an [enrollment payment method object](https://docs.y.uno/reference/enroll-payment-method-checkout) on your **server-side** to keep private API keys secure; define which payment method the customer can enroll when creating the payment method object.
...
| Parameter                         | Description                                                                                                                                                                                                                                       |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `customerSession`                 | Refers to the current enrollment's [customer session](https://docs.y.uno/reference/create-customer-session). Example: `e15648b0-fcd5-4799-a14c-cc463ae8a133`.                                                                                                              |
...
To start the enrollment process, provide the `customer_session`. Create a customer using the [Create Customer](https://docs.y.uno/reference/create-customer) endpoint. In the response, you'll receive the customer `id`, which you use to create the customer session.

After creating the customer, you can create the customer session. Use the customer `id` and the [Create Customer Session](https://docs.y.uno/reference/create-customer-session) endpoint. The `customer_session` will be provided in the response. You need a new `customer_session` every time you enroll in a new payment method.

### Step 4: Create an enrollment payment method object

Create an enrollment payment method with the [Enroll Payment Method](https://docs.y.uno/reference/enroll-payment-method-checkout) endpoint and set which method the customer can enroll (Headless supports CARD only). To verify cards (zero-value authorization) before enrollment, include the `verify` object when creating the payment method object.
...
| `customerSession` | Refers to the current customer's [customer session](https://docs.y.uno/reference/create-customer-session). Example: `'eec6578e-ac2f-40a0-8065-25b5957f6dd3'`                                                                                                                                                 |
