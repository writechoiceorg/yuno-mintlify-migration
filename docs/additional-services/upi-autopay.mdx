...
Implement Autopay through the `stored_credentials` field available in endpoints like [Create payment](/docs/create-payment). The full path for create payments is `payment_method.detail.detail.bank_transfer.stored_credentials`.
