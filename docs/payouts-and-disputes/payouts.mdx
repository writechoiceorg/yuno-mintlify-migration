...
Learn more about the payout status on the [Payouts Status](https://docs.y.uno/reference/payout-workflow) page.
...
* You need to integrate with [Yuno API](https://docs.y.uno/reference/create-payout).
...
2. Create the payout using the [Create Payout](https://docs.y.uno/reference/create-payout) endpoint. You need to provide the beneficiary information, the amount, and the withdrawal method to make the payout.
...
4. To check the current status of payouts, you can use one of the following options:
   * The [Retrieve Payout by ID](https://docs.y.uno/reference/retrieve-payout-by-id) endpoint, informing the `id` received after creating the payout.
   * The [Retrieve Payout by Merchant Reference](https://docs.y.uno/reference/retrieve-payout-by-merchant-reference) endpoint, informing the `merchant_reference` provided when creating the payout.
...
In order to be able to use this, you'll need to first charge the customer using one of our PCI compliant solutions (Any [Yuno SDK](https://docs.y.uno/docs/quickstart)) and then make a Payout indicating the original transaction where we can find the credit card information.
