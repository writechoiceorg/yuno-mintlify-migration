...
Understanding the reason code behind each chargeback is crucial, as it explains why the customer initiated the dispute. Familiarity with these codes allows you to tailor your response more effectively and gather the necessary information for each case. In the [Chargeback Reason Codes section](https://docs.y.uno/docs/reason-codes), you can find a comprehensive list of all possible codes provided by acquirers. In each chargeback transaction you'll be able to find the reason code in the `response_code` field.
...
If you need to add additional evidence to an existing dispute, you can use the [Update Dispute](https://docs.y.uno/reference/update-dispute) endpoint. This allows you to provide supplementary documentation after the initial dispute has been submitted.
...
* **Retrieving dispute information**: You can retrieve the current status and information about a chargeback/dispute by using the [Retrieve Payment by ID](https://docs.y.uno/reference/retrieve-payment-by-id) endpoint, which includes transaction details with chargeback information.
</Info>

Please refer to the [Disputes API](https://docs.y.uno/reference/disputes) and [Update Dispute API](https://docs.y.uno/reference/update-dispute) reference sections for more information.