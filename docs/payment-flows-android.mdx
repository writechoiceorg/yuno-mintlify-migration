...
* Create a `checkout_session` and the payment via the API; create a customer using the [Create customer endpoint](https://docs.y.uno/reference/create-customer) before creating a payment
...
Each payment requires a new `checkout_session`. Use the [Create checkout session](https://docs.y.uno/reference/create-checkout-session) endpoint to create one; use that session to initiate the payment.
...
<Frame>![Seamless (payment Android) screenshot](/images/reference/payment-flows-android/image1.png)</Frame>

See [Requirements](#requirements) above.

### Step 1: Create a customer

Create a customer using the [Create customer endpoint](https://docs.y.uno/reference/create-customer) before initiating payments. This step is required to:
...
Create a new `checkout_session` using the [Create checkout session](https://docs.y.uno/reference/create-checkout-session) endpoint to initialize the payment flow. Make sure to:
...
| `callbackPaymentState` | Returns the current payment process state (e.g., `SUCCEEDED`, `FAIL`, `PROCESSING`, `REJECT`), see [payment status reference](https://docs.y.uno/reference/payment). Optional if you don't need the result. |
...
See the [Payment Status reference](https://docs.y.uno/reference/payment) to understand how SDK states relate to the full payment status and sub-status returned by the API.
...
| `callbackPaymentState` | Optional function handling state updates. See [payment status reference](https://docs.y.uno/reference/payment) for mapping. |
...
Import Yuno and provide a valid **PUBLIC\_API\_KEY** to initialize Headless (Android):
...
Call the `apiClientPayment` function after your customer selects a payment method to start the checkout process. This function requires configuration parameters and initiates the collection of information needed for 3DS authentication and fraud prevention tools configured in your [routing](https://docs.y.uno/docs/routing).
