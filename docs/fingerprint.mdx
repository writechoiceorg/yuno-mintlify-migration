...
Card fingerprints are a cryptographic hash of the card's PAN that uniquely identifies a specific payment card. Yuno returns this value as `fingerprint` on the [Payment Method object](https://docs.y.uno/reference/the-payment-method-object-api). When the same card is enrolled multiple times by the same customer, Yuno creates separate enrollments with distinct `vaulted_token` values, and the fingerprint helps you detect duplicates.
...
Use the [Retrieve Enrolled Payment Methods](https://docs.y.uno/reference/retrieve-enrolled-payment-methods-api) endpoint to list the customer's existing cards.
