...
<Tip>
**Using PayPal for subscriptions?** A one-time `PAYPAL` payment (even with `vault_on_success: true`) does not create a billing agreement. See [Set Up PayPal for Subscriptions](/docs/payment-features/enrollment/paypal-subscriptions) for the `PAYPAL_ENROLLMENT` flow required for recurring billing.
</Tip>
