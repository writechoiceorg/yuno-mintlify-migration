...
In this section, we explore how the `split_marketplace` object is used to divide a [payment](/reference/create-payment) among multiple recipients. This object is an array where each entry specifies a recipient and their corresponding share of the payment.
...
* If using multiple payment providers for split payments, we recommend utilizing the [recipients object](/reference/create-recipients), as it allows defining more than one provider for each recipient.
