...
* **Stored Credentials**: You control the recurrence. You're responsible for sending each transaction according to your own schedule. See [Stored Credentials](https://docs.y.uno/docs/stored-credentials).
</Note>
...
Learn more about the status of the subscription on the [Subscription Status](https://docs.y.uno/reference/status-subscriptions) page.
...
* **Create**: Use the [Create a Subscription endpoint](https://docs.y.uno/reference/Create-Subscription) to subscribe a customer.
* **Pause**: Pause an active subscription with the [Pause Subscription endpoint](https://docs.y.uno/reference/Pause-Subscription).
* **Resume**: Resume a previously paused subscription with the [Resume Subscription endpoint](https://docs.y.uno/reference/Resume-Subscription).
* **Cancel**: Cancel an active subscription with the [Cancel Subscription endpoint](https://docs.y.uno/reference/Cancel-Subscription).
* **Retrieve**: Use the [Retrieve Subscription endpoint](https://docs.y.uno/reference/Retrieve-Subscription) to get the details of a subscription.
...
1. To begin, [Create a Customer](https://docs.y.uno/reference/Create-Customer). You will need to provide personal customer information and the **merchant\_customer\_id**, a unique identifier for the customer used in your system. Upon completing the customer creation process, you will receive an **id** that identifies the user within the Yuno system. This **id** will be used to create the subscription.
2. After creating the customer, you will need to [enroll a credit card](https://docs.y.uno/docs/enroll-payment-methods) to generate a **vaulted\_token** for use in the subscription creation.
3. [Create a Subscription](https://docs.y.uno/reference/Create-Subscription) for your customer and the enrolled card. At this step, you will configure the subscription and define the payment method. When creating the subscription, you can customize:
...
In the response to the [Create Subscription](https://docs.y.uno/reference/Create-Subscription) endpoint, you receive an `id` which is used to identify the created subscription. You will use the `id` if you decide to pause, resume, or cancel the subscription.