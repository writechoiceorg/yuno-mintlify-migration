...
Create a customer in Yuno's system using the [Create customer endpoint](https://docs.y.uno/reference/create-customer) before enrolling payment methods. This endpoint will return a `customer_id` that you'll use to associate enrolled payment methods with the specific customer.

Then create a customer session using the [Create Customer Session](https://docs.y.uno/reference/create-customer-session) endpoint. The session information will be used when calling the enrollment methods.
