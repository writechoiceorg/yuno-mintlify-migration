...
To perform an AFT, you will need to send a request to our [Payments API](https://docs.y.uno/reference/create-payment) with the `additional_data.order.account_funding` object specifying the `sender` and`beneficiary` data.
...
For more details, refer to the create [payments API](https://docs.y.uno/reference/create-payment) section.

<Note>
**Additional Data**

In case you need to define additional information about the seller, such as `merchant_category_code`for example, you can find extra fields in the seller\_details inside the [additional\_data struct](https://docs.y.uno/reference/the-payment-object) in the payment.
</Note>