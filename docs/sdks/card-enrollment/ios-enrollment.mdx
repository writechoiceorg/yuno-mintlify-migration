...
* Create a customer using the [Create customer endpoint](/reference) before enrolling
...
Create a customer in Yuno's system using the [Create customer endpoint](/reference) before enrolling payment methods. This endpoint will return a `customer_id` that you'll use to associate enrolled payment methods with the specific customer.
...
Including the library in your project is done in the same way as in payment flows. Follow the steps in [Include the library in your project](../full-checkout/ios-payments#include-the-library-in-your-project) there.
