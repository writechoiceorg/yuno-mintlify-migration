...
Parameters for payment flows. All parameters used in [Payment flows (iOS)](../full-checkout/ios-payments) are listed here with full detail.
...
Runtime behavior and appearance are configured via `YunoConfig` when calling `Yuno.initialize(apiKey:config:)`. All parameters used across iOS payment and enrollment flows are listed below. For visual styling (fonts, colors, buttons), see [SDK customizations (iOS)](../customization/ios).

| Parameter               | Type       | Required | Description                                                                                                                                                                                                                                     |
| ----------------------- | ---------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `appearance`            | Appearance | No       | Custom appearance (fonts, colors, buttons). Use `Yuno.Appearance(...)` to set `fontFamily`, `accentColor`, `buttonBackgroundColor`, `buttonTitleColor`, and related fields. See [SDK customizations (iOS)](../customization/ios) for all options. |
...
Parameters for enrollment flows. All parameters used in [Enrollment flows (iOS)](../../card-enrollment/ios-enrollment) are listed here with full detail.
