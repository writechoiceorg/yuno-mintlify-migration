...
* Access your [API credentials](https://docs.y.uno/docs/developers-credentials) on the Yuno Dashboard, which are composed by:
  ...
  * `private-secret-key`
  * `account_id`
* [Set up your connections](https://docs.y.uno/docs/connections) on your Yuno Dashboard account.
* [Build a route](https://docs.y.uno/docs/routing) for the payment method to define how it will be processed.
* [Configure the checkout builder](https://docs.y.uno/docs/checkout-builder) to make your connected payments available.
...
Use the [Create Checkout Session](https://docs.y.uno/reference/create-checkout-session) endpoint. Notice that the `customer_id` required to perform the request is the `id` you received when creating the customer in [Step 1](https://docs.y.uno/docs/create-payment-basic#1-create-a-customer).
...
You can always use the list [Payment type](https://docs.y.uno/reference/payment-type-list) to check all available payment types. In the response of the SDK, you will receive the `one-time-token`, which you will use to create the payment.

<Note>
**Testing credit card payments**

Remember that for testing credit card payments you can set the [Yuno Test Payment Gateway](https://docs.y.uno/docs/yuno-testing-gateway) as a provider in your Card route.
</Note>

### Step 4: Create a payment

You will create a payment using the endpoint [Create Payment](https://docs.y.uno/reference/create-payment). With Yuno, you can create payments with several payment methods, using 3DS or split payments. However, this guide focuses on a simple payment without additional authentication, validation, or enrollment requirements.
