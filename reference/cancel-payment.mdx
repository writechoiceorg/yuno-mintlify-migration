...
openapi: "/openapi.json POST /v1/payments/{payment_id}/transactions/{transaction_id}/cancel"
---
...
If you need a receipt for the canceled transaction, you can retrieve it after it has been created. To do this, use the [Retrieve Payment by ID](https://docs.y.uno/reference/retrieve-payment-by-id) endpoint and check the `receipt_url` field in the `payment.transaction` object. See [The Payment Object](https://docs.y.uno/reference/the-payment-object) for `receipt`, `receipt_url`, and `receipt_language`.

To use this endpoint, you have to provide an `X-Idempotency-Key` in your request. Check the [Authentication](https://docs.y.uno/reference/authentication#idempotency) page for more information.
