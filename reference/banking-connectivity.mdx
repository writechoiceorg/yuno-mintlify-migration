...
1. **Create an entity**: Register an individual or business using [Create Entity](https://docs.y.uno/reference/create-entity).
2. **Onboard the entity**: Submit KYC/KYB documentation through [Create Entity Onboarding](https://docs.y.uno/reference/create-entity-onboarding).
3. **Wait for onboarding approval**: Monitor the onboarding status via [Get Entity Onboarding Status](https://docs.y.uno/reference/get-entity-onboarding-status) or listen for webhook events.
4. **Create a bank account**: Once onboarding succeeds, open an account with [Create Account](https://docs.y.uno/reference/create-account-banking).
5. **Initiate transfers**: Send funds using [Initiate Entity Transfer](https://docs.y.uno/reference/initiate-entity-transfer).
...
Responses from [Create Account](https://docs.y.uno/reference/create-account-banking) include region-specific banking identifiers:
...
Yuno sends webhook notifications to your configured endpoint for the following events. For general webhook setup, delivery behavior, and retry logic, see [Webhooks Overview](https://docs.y.uno/docs/webhooks#banking-connectivity-webhook-events).
...
Incoming transfer notifications are sent to your endpoint at `{merchant_base_URL}/v1/banking/transfers`. See [Webhook Notifications](https://docs.y.uno/reference/webhook-notifications-banking) for the payload structure.
