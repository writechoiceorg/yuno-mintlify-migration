...
Check the [3D Secure](/docs/3d-secure) page to learn how to create payments using 3DS on Yuno.
</Note>
...
To test all possible transaction outcomes within Yuno in **Sandbox**, please refer to the following [documentation](/docs/yuno-testing-gateway#44-provide-the-payment-method-information) for **test data**.

<Danger>
**Metadata and routing logic**

If you use metadata to drive routing logic, it must also be set in the **[Checkout Session](/reference/create-checkout-session)**. Filling it only in the Payment object will not activate route logic.
</Danger>
...
1. Create an authorization using the [Authorize Payment](/reference/authorize-payment) request.
2. Capture the created authorization using the [Capture Authorization](/reference/capture-authorization) request.
