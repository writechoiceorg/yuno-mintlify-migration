...
<Note>
`PREVENTED` is a transaction status only for `type = CHARGEBACK` when the provider/network reports a [predispute deflection](https://docs.y.uno/docs/chargeback-management). In these scenarios, only the chargeback webhook is sent; no refund webhook is emitted.
