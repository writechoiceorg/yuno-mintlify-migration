...
openapi: "/openapi.json POST /v1/customers/sessions/{customer_session}/payment-methods"
---
...
Note that this request requires an `X-Idempotency-Key`. Check the [Authentication](https://docs.y.uno/reference/authentication#idempotency).
...
When you retrieve the payment methods available to enroll, only the ones which you have connected, created a route, and added to your checkout will be present. For more information about enrollment workflows, see the [Enroll Payment Methods](https://docs.y.uno/docs/enroll-payment-methods) guide.
</Note>
