...
We have launched a new product that allows merchants to execute [Payouts](https://docs.y.uno/docs/payouts) through the integration of various providers.
...
Please refer to the [technical documentation](https://docs.y.uno/docs/payouts) for more info.

## Payment method fingerprint

We've added a new field to the [Payment method](https://docs.y.uno/docs/enroll-payment-methods#step-5-retrieve-payment-methods) object called `fingerprint`. It is a field that represents your customer's card throughout your organization. When a customer enrolls a credit card multiple times related to one or many Yuno accounts, multiple `vaulted_tokens` will be generated, but the fingerprint lets you identify when the same card is used for the customer.
